...
# 版本更新&预更新内容
    ...
    V4.2（最高档充电提早获取）
    1.模拟用户操作，在抖音商城中进行浏览、下单流程（创建订单但不付钱）
    2.模拟用户操作，到停止挂机时间段退出抖音，到开始挂机时间再启动抖音
    3.模拟用户操作，在视频首页刷一会视频，并随机加评论、点赞
    4.优化是否存在福袋的逻辑，只检测固定间隔的位置
    5.调整了凌晨关闭抖音后等待时长的逻辑，统一等待到8点继续挂机
    6.修复了福袋内容不想要多次切换直播间后，后续无法参与新福袋抽奖的问题
    ...
    1.兼容最新版本的paddleOCR    
    2.移除偏移值的设定，改为动态自动查询
    3.将设备初次识别成功的顶部状态栏高度存下来，后续运行改成直接读取保存的值
    4.优化福袋参与抽奖按钮文字的二次判定逻辑
    5.调整人机验证弹窗的判定
    6.优化到凌晨时段，无法关闭抖音，卡在了直播关注列表的情况
